 ...
 */
 ...
 metadata {
	definition (name: "Xiaomi Aqara Wireless Switch", namespace: "bspranger", author: "bspranger", minHubCoreVersion: "000.022.0002", ocfDeviceType: "x.com.st.d.remotecontroller") {
		...
		// Aqara Smart Light Switch - dual button - model WXKG07LM (2020 revision)
  		fingerprint deviceId: "5F01", inClusters: "0000,0003,0019,0012,FFFF", outClusters: "0000,0003,0004,0005,0019,0012,FFFF", manufacturer: "LUMI", model: "lumi.remote.b286acn02", deviceJoinName: "Aqara Switch WXKG07LM (2020)"		
...
def setNumButtons() {
	if (device.getDataValue("model")) {
		...
		if (!state.numButtons || state.numButtons == 7) {
			...
			else if (modelName.startsWith("lumi.remote.b286acn02")) {
			modelText = "Wireless Smart Light Switch WXKG07LM - dual button (2020 revision)"
			state.numButtons = 6
			}
